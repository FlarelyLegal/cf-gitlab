...
| `notify-admin-granted.rb` | user_update_for_admin                                       | Discord alert when a user is granted admin privileges      |
| `auto-label-projects.rb`  | project_create                                              | Creates 32 industry standard labels on every new project   |
...
All hooks read from `/root/.secrets/gitlab.env`:
...
| `notify-admin-granted.rb` | `DISCORD_WEBHOOK_URL_ADMIN`       |
| `auto-label-projects.rb`  | `GITLAB_API_TOKEN_HOOKS`          |
